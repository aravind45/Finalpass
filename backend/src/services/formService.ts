...
export class FormService {
    ...
    private metadata: { forms: FormMetadata[] } | null = null;
     ...
     * Load form metadata
     */
    async loadMetadata(): Promise<{ forms: FormMetadata[] }> {
        if (this.metadata) {
     ...
     * Get form by ID
     */
    async getFormById(formId: string): Promise<FormMetadata | null> {
        const metadata = await this.loadMetadata();
     ...
     */
    async getFormsByInstitution(institution: string): Promise<FormMetadata[]> {
        const metadata = await this.loadMetadata();
        return metadata.forms.filter(f => 
            f.institution.toLowerCase() === institution.toLowerCase()
        );
    }
     ...
     */
    async fillForm(formId: string, data: FormFillData): Promise<Buffer> {
     ...
     */
    async autoPopulateForm(formId: string, estateData: any, assetData?: any): Promise<FormFillData> {
        const form = await this.getFormById(formId);
        if (!form) {
            throw new Error(`Form not found: ${formId}`);
        }
        ...
        for (const field of form.fields) {
            if (!field.estateField) continue;

            try {
                const value = this.getNestedValue(
                    { ...estateData, asset: assetData },
                    field.estateField
                );
     ...
     */
    async mergePDFs(coverPage: Buffer, formPdf: Buffer): Promise<Buffer> {
     ...
     * Validate form data
     */
    async validateFormData(formId: string, data: FormFillData): Promise<{ valid: boolean; errors: string[] }> {
        const form = await this.getFormById(formId);
        if (!form) {
